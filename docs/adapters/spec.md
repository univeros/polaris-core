...
## 6b. Univeros (WP4, retired 2026-09-10)

Added on 2026-09-10 at the owner's request as `packages/univeros` (`polaris/univeros`, with `univeros/polaris` 2.0 as a wrapper) and retired the same day, before any code, by the owner's second decision: the Univeros module is built as **`univeros/polaris` 2.0 in its own repository**, on `polaris/core`, `polaris/psr15`, `polaris/pdo` and `polaris/cli`, like any application. Univeros is the owner's own framework, and its module wants framework-native surfaces (`#[Command]` classes for `bin/altair`, Doctor checks, MCP tools, the Action/Input/Responder shape) that §0.1 forbids in an adapter here; keeping it there also keeps this repository free of `Altair\`, `Cycle\` and `Univeros\` with no exemption, which was the point of the extraction. No `examples/univeros` either: the demo goes with the module (PR #17 closed unmerged).

What this repository hands over is `docs/adapters/univeros-polaris-2.0.md`: the 2.0 `composer.json`, the module's design as it was verified against `univeros/framework` 2.5.1 (the skeleton's front controller already merges module middleware, `vendor/bin/altair` boots a bare container so the application ships its own entry point, Cycle's driver keeps its PDO private, `Altair\Cli\Application` extends Symfony Console), the recipe that replays the functional suite and the 184 fixtures through Relay with `POLARIS_HARNESS`, and the CHANGELOG/UPGRADE text (2.0 is a new major, TOTP factors are re-enrolled, 1.x keeps working). The owner's decisions 1 to 4 of 2026-09-10 (`decisions.md`) apply in that repository.
...
| **WP4** | Univeros (retired) | §6b | No code in this repository. `docs/adapters/univeros-polaris-2.0.md` holds what the owner applies in `univeros/polaris`: the 2.0 `composer.json`, the module design, the harness recipe, the CHANGELOG/UPGRADE text |
...
Branch names: `adapters/wp0` (split across PRs #12 and #13 as noted), `adapters/wp1` … `adapters/wp5`, one at a time, each merged before the next starts (stacking while CI runs is fine). WP4 (Univeros) was added on 2026-09-10 at the owner's request and retired the same day (§6b); the client keeps its number.

---

## 9. Definition of done (task 2)

- Three adapter packages (Laravel, Symfony, Yii), each proving the frozen contract through its own HTTP kernel in CI; framework namespaces isolated by `bin/check-imports`; every adapter's demo green in CI from a clean clone; `univeros/polaris` 2.0 prepared for the owner as a document (§6b).
...
The plugin contract (`target-design.md` §3.2) and everything built on it: the Agents plugin, the Cloud connect plugin, social login, passkeys, magic links (task 3, its own spec) · dedicated `polaris/doctrine` or `polaris/eloquent` database adapters (the frameworks' connections are used through `polaris/pdo`; a dedicated adapter only if friction proves the need, logged) · React/Vue hooks for the client · any change to token formats, session semantics, the 52 endpoints, or `docs/auth/api-reference.md` · a Cycle adapter · a Univeros adapter or demo (§6b) · **any change to `univeros/polaris` from this repository** (its 2.0 is prepared as a document for the owner).
