...
polaris-core/
  packages/
    ...
    univeros/    polaris/univeros  Polaris\Univeros\  module (bindings, middleware, migrations), the token bridge, bin/altair commands
  ...
  examples/
    ...
    slim/  laravel/  symfony/  yii/  univeros/   one minimal host per adapter; each bin/walkthrough.sh runs the shared script
...
| `univeros` | `core`, `psr15`, `pdo`, `cli`, the `univeros/*` packages it imports (`container`, `module`, `http`, `persistence` for the Cycle connection and migration contracts, `cli` for the commands), each `^2.5` |
...
`packages/univeros`, `Polaris\Univeros\`, Univeros 2 (`univeros/framework ^2.5`; the package requires the `univeros/*` packages it imports). Requested by the owner on 2026-09-10; `univeros/polaris` 2.0 becomes a wrapper of this package (a meta-package requiring it), 1.x installations are not touched, 2.0 is announced as a breaking change with no data-compatibility layer.

- **`Module`** (`Altair\Module\Contracts\ModuleInterface`, `MiddlewareProviderInterface`, `MigrationDirectoriesProviderInterface`), registered in the application's `config/modules.php`: `apply(Container)` builds Polaris from the application's configuration (the `auth` namespace through `univeros/configuration` where present, the secrets from the environment as `EnvironmentConfig` reads them) on the framework's connection (the Cycle database's PDO handle through `univeros/persistence`, or `POLARIS_DSN` when the application has no ORM configured) through `polaris/pdo`, the container's PSR-14 dispatcher with `Polaris::listeners()` subscribed, the container's PSR-16 cache and PSR-3 logger, and binds `Polaris`, `Graph`, `Pipeline`, `PolarisMiddleware`, and the framework's `Altair\Http\Contracts\TokenFactoryInterface` to a bridge over `Graph::tokenFactory()` (the 1.0 `AltairTokenFactoryBridge` and `DualToken`, back where they belong), so the application's own routes use the framework's `TokenAuthenticationMiddleware` with Polaris access tokens, as in 1.0. Every port takes the container's binding when present, else core's default.
- **Routes**: `PolarisMiddleware`, first in the Relay pipeline, serves every manifest path through `Pipeline::handle()` (a JSON body the framework has not parsed is parsed here) and lets every other path continue to the framework's dispatcher. It runs ahead of the framework's `ExceptionHandlerMiddleware` because that middleware rewrites every error response into problem+json and Polaris answers its own envelopes; for the same reason the Polaris routes are not registered in the FastRoute table (they would never be dispatched), `bin/altair polaris:manifest` lists them. `middleware()` declares it at a priority below `MiddlewarePriority::EXCEPTION_HANDLER` for hosts that consume module middleware; the skeleton adds it to its Relay by hand (the demo shows the line). (Owner's decision 2: "do what you think is best".)
- **Migrations** (owner's decision 4): `migrationDirectories()` points at the package's `migrations/`, one Cycle migration whose `up()` calls `Polaris\Pdo\SchemaInstaller::create()` on the migrator's database handle and whose `down()` drops, so `bin/altair db:migrate` installs Polaris; `polaris schema:diff` proves parity.
- **Console**: the six `polaris/cli` commands as `polaris:*` through whatever `univeros/cli` offers a module (its command locator scans directories); if the framework has no module hook for commands, the package documents `vendor/bin/polaris` and the WP logs it.
- **Mail**: Univeros ships no mailer package; `mailer` is `log` or an `OtpMailerInterface` binding. No bridge.
- **1.0 data** (owner's decision 3): no compatibility encrypter and no re-encryption path; `docs/adapters/univeros-polaris-2.0.md` carries what the owner applies in the `univeros/polaris` repository (never modified from here): the 2.0 `composer.json` requiring `polaris/univeros`, and the CHANGELOG/UPGRADE text stating that 2.0 is a new major on `polaris/core`, that TOTP secrets encrypted by 1.x are not readable (re-enrolment), and that 1.x keeps working unchanged.
- **`Tests\Harness`**: an `Altair\Container\Container` with the module applied on the test's instances (bound in the container before `apply()`, the module takes what is bound), the skeleton's Relay pipeline (`PolarisMiddleware`, `ExceptionHandlerMiddleware`, `DispatcherMiddleware`, `ActionMiddleware`); no transport headers (Relay adds nothing).
- **`examples/univeros`** (PR #17's branch reworked): `config/modules.php` registers `Polaris\Univeros\Module`, `public/index.php` is the skeleton's with the middleware first, a `/app/me` action behind the framework's `TokenAuthenticationMiddleware` fed by the bridge, `bin/setup` through `bin/altair db:migrate` (or `polaris schema:create` if the persistence package cannot run on SQLite), the shared walkthrough, the `demo-univeros` CI job.
- **`bin/check-imports`**: `Altair\`, `Cycle\` and `Univeros\` allowed only under `packages/univeros` and `examples/univeros`, forbidden everywhere else; `CheckImportsTest` covers it.

---

## 7. TypeScript client (WP5)
...
| **WP4** | Univeros | §6b | (a)–(g) as WP1 for `packages/univeros`, `Polaris\Univeros\Tests\Harness`, `examples/univeros`; plus (h) `bin/check-imports` allows `Altair\`/`Cycle\`/`Univeros\` only under `packages/univeros` and `examples/univeros` (unit-tested); (i) `bin/altair db:migrate` installs Polaris in the demo and `polaris schema:diff` is clean; (j) the demo's `/app/me` answers 200 with the user for a Polaris access token through the framework's `TokenAuthenticationMiddleware`; (k) `docs/adapters/univeros-polaris-2.0.md` holds the 2.0 wrapper's `composer.json` and its breaking-change note |
| **WP5** | TypeScript client | §7 | `npm ci && npm run generate && git diff --exit-code src/schema.d.ts && npm run typecheck && npm test` green in CI against the Slim demo |

Branch names: `adapters/wp0` (split across PRs #12 and #13 as noted), `adapters/wp1` … `adapters/wp5`, one at a time, each merged before the next starts (stacking while CI runs is fine). WP4 (Univeros) was added on 2026-09-10 at the owner's request and takes precedence over the client.

---

## 9. Definition of done (task 2)

- Four adapter packages (Laravel, Symfony, Yii, Univeros), each proving the frozen contract through its own HTTP kernel in CI; framework namespaces isolated by `bin/check-imports`; every adapter's demo green in CI from a clean clone; the `univeros/polaris` 2.0 wrapper prepared for the owner.
...
The plugin contract (`target-design.md` §3.2) and everything built on it: the Agents plugin, the Cloud connect plugin, social login, passkeys, magic links (task 3, its own spec) · dedicated `polaris/doctrine` or `polaris/eloquent` database adapters (the frameworks' connections are used through `polaris/pdo`; a dedicated adapter only if friction proves the need, logged) · React/Vue hooks for the client · any change to token formats, session semantics, the 52 endpoints, or `docs/auth/api-reference.md` · a Cycle adapter · **any change to `univeros/polaris` from this repository** (the 2.0 wrapper is prepared as a document for the owner).
